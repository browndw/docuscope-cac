...
def apply_logging_patch():
    ...
    if not _config.is_cli:
        raise ValueError(
            ...
            "not to be used within docuscope while it is in library mode "
            "as monkey patches on libraries/modules external to docuscope "
            "can be exceptionally confusing to a library user. "
            "This is only to be used when docuscope is being called via "
            "the docuscope CLI."
        )
