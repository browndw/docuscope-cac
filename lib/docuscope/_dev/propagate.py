...
from docuscope._imports import black, tomlkit
...
def _propagate_requirements():
    ...
    for extras, filename, include_docuscope, editable in REQUIREMENTS_CONFIG:
        _make_requirements_txt(
            extras=extras,
            filename=filename,
            include_docuscope=include_docuscope,
            editable=editable,
        )
...
def _make_requirements_txt(
    ...
    editable : bool
        Whether or not the docuscope install should be 'editable'.
    """
    if not include_docuscope and editable is not None:
        raise ValueError("Setting editable only works if using `include_docuscope`")

    if include_docuscope and editable is None:
        raise ValueError("When using `include_docuscope` need to also set `editable`")
    ...
    if include_docuscope:
        docuscope_install_command = f".[{','.join(extras)}]\n"
        if editable:
            docuscope_install_command = f"-e {docuscope_install_command}"

        with open(filepath, "a") as f:
            f.write(docuscope_install_command)
