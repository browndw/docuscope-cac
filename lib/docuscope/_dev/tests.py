...
from docuscope._imports import pytest, tabulate, tqdm

import docuscope._utilities.test as pmp_test_utils
import docuscope.tests.e2e.utilities as cypress_test_utilities
...
def run_clean_imports(_):
    ignore_scopes = [
        ...
        # see <https://github.com/docuscope/docuscope/issues/1382>
        "docuscope._experimental.pedromartinez",
        "docuscope._experimental.paulking",
    ]
    tests_scopes = ["docuscope.conftest", "docuscope.tests"]
...
def _import_and_print(python_executable, import_paths):
    ...
    for import_path in tqdm.tqdm(import_paths):
        try:
            ...
            # flag:
            #
            #      `docuscope dev imports --no-isolation`
            #
...
def _get_problem_module_and_line_number(error_text):
    ...
    module = (
        re.search(r"(docuscope.*)\.py", relevant_line)
        .group(1)
        .replace(os.sep, ".")
        .replace("-", "_")
    )
...
def _call_pytest(remaining, label):
    ...
    try:
        retcode = pytest.main(["--pyargs", "docuscope"] + remaining)
    finally:
