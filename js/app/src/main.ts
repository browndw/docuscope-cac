...
if (env.name === "development") {
  appStreamlitServer = spawn("poetry", ["run", "docuscope", "gui", "--electron"]);

  // appStreamlitServer = spawn("./docuscope",  ["gui", "--electron"], {
...
app.on("ready", () => {
  ...
  streamlitPortDelegate.promise.then((port) => {
    const docuscopeAppUrl = url.format({
      pathname: `localhost:${port}`,
      protocol: "http:",
      slashes: true,
    });

    mainWindow.loadURL(docuscopeAppUrl);
  });
});
